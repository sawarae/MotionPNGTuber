...
class App(tk.Tk):
    ...
    def on_preview_erase_range(self) -> None:
        ...
        NOTE: OpenCVのGUI関数(imshow/waitKey)はメインスレッドで実行する必要があるため、
        ワーカースレッドではなく直接実行する。
        """
        try:
            import cv2  # type: ignore
        ...
        # ボタンを直接無効化（キュー経由だと遅延するため）
        self.btn_erase_range.configure(state="disabled")
        ...
        try:
            ...
            try:
                ...
                pass
            # ボタンを再有効化
            self.btn_erase_range.configure(state="normal")
