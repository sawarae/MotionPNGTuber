...
| 日付 | 内容 |
|------|------|
| 2026/01/19 | **Gradioインターフェース**追加、ブラウザベースの操作UI |
...
| 🌐 Gradio UI | ブラウザベースの直感的な操作インターフェース |
...
- 📹 動画ファイルのアップロード & サンプル選択
- 👄 mouth画像フォルダの簡単選択
- ⚙️ パラメータ設定（感情自動判定、オーディオデバイス等）
- 🎮 ワンクリックで実行開始/停止

#### 使い方

1. ブラウザで http://localhost:7860 にアクセス
2. 動画を選択（アップロードまたはサンプルから選択）
3. mouth画像フォルダを選択
4. パラメータを調整（オプション）
5. 「実行開始」ボタンをクリック
6. OpenCVウィンドウでプレビューを確認（'q'キーで終了）
